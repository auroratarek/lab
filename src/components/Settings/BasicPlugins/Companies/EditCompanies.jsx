...
export default function EditCompanies({ id, open, setOpen }) {
  ...
  useEffect(() => {
    setEditedName(
      CompaneisSelector
        ? CompaneisSelector[
            CompaneisSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.name
        : ""
    );
  }, [id]);
  ...
  const Edit = async () => {
    ...
    await axios
      .put(
        `lab-scope/company-update?company_id=${id}&name=${
    ...
    // let content = CompaneisSelector.map((item) => {
    //   return item.id == id
    //     ? { id: id, name: document.getElementById("Company").value }
    //     : item;
    // });
    // dispatch(addAllCompany(content));
    // setOpen(false);
  };
