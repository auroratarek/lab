...
export default function DeleteUnit({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/delete-TestUnit?test_unit_id=${id}`)
      .then((response) => {
        GetTestUnits();
        setOpen(false);
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500)
          setError("this test Unit used in analysis");
      });
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Unit?")} :{" "}
                            <span className="text-black ml-5">
                              {" "}
                              {
                                SelectUnit[
                                  SelectUnit.findIndex(
