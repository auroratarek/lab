...
export default function EditUnit({ id, open, setOpen }) {
  ...
  function close() {
    setEditedName(
      SelectUnit
        ? SelectUnit[SelectUnit?.findIndex((unitItem) => unitItem.id === id)]
            ?.unit
        : ""
    );
    setOpen(false);
  ...
  const Edit = async () => {
    ...
    await axios
      .put(
        `lab-scope/update-TestUnit?test_unit_id=${id}&test_unit=${editedName}`
      )
      .then((response) => {
        setReady(true);
    ...
    // let content = SelectUnit.map((item) => {
    //   return item.id == id
    //     ? { id: id, name: document.getElementById("EditUnitName").value }
    //     : item;
    // });
    // dispatch(addAllTestUnit(content));
    // setOpen(false);
  };
                ...
                <Dialog.Panel className="relative px-10 pb-8 bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-2 sm:max-w-xl sm:w-full ">
                  ...
                  <div className="bg-white  py-3  flex sm:flex-row-reverse justify-between mt-10">
                    <button
                      type="button"
                      disabled={!ready}
                      className={`flex flex-grow font-medium text-sm py-3 ${
