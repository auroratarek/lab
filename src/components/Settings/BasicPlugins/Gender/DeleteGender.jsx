...
import { addAllGender, removeFromGender, selectGenders } from "./GenderSlice";
...
export default function DeleteGender({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/gender-delete?gender_id=${id}`)
      .then((response) => {
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500) setError("this gender used ");
      });
  };
  return (
    <div>
