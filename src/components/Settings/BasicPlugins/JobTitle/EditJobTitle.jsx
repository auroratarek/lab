...
export default function EditJobTitle({ id, open, setOpen }) {
  ...
  function close() {
    setEditedName(
      JobTitlesSelector
        ? JobTitlesSelector[
            JobTitlesSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.name
        : ""
    );
    setOpen(false);
  ...
  const Edit = async () => {
    setReady(false);
    await axios
      .post(`lab-scope/job-title-update/${id}`, {
        name: document.getElementById("EditJobTitle").value,
      })
      .then((response) => {
        GetJobTitles();
        setReady(true);
