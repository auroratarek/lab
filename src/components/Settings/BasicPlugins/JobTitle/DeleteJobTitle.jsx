...
export default function DeleteJobTitle({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/job-title-delete/${id}`)
      .then((response) => {
        GetJobTitles();
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500) setError("this job title used ");
      });
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Job Title?")} :{" "}
                            <span className="text-black ml-5">
                              {
                                JobTitlesSelector[
                                  JobTitlesSelector.findIndex(
