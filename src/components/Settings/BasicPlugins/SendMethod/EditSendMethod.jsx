...
export default function EditSendMethod({ id, open, setOpen }) {
  ...
  function close() {
    ...
    setEditedName(
      SendMethodsSelector
        ? SendMethodsSelector[
            SendMethodsSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.name
        : ""
    );
    setOpen(false);
  ...
  const Edit = async () => {
    ...
    await axios
      .put(
        `lab-scope/sendMethod-update?send_method_id=${id}&name=${editedName}`
      )
      .then((response) => {
        GetSendMethods();
        setReady(true);
