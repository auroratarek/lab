...
import {
  addAllSendMethod,
  removeFromSendMethod,
  selectSendMethods,
} from "../../../../GlobalData/Basic Plugins/SendMethodSlice";

export default function DeleteSendMethod({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/sendMethod-delete?send_method_id=${id}`)
      .then((response) => {
        GetSendMethods();
        setOpen(false);
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500) setError("this send method used");
      });
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Send Method?")} :{" "}
                            <span className="text-black ml-5">
                              {
                                SendMethodsSelector[
                                  SendMethodsSelector.findIndex(
                          ...
                          <span className="text-red-600 tex-sm">{error}</span>
                        </div>
                      </div>
                    </div>
                  </div>
