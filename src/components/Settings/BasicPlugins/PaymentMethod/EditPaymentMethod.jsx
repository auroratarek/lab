...
export default function EditPaymentMethod({ id, open, setOpen }) {
  ...
  useEffect(() => {
    setEditedName(
      PaymentMethodsSelector
        ? PaymentMethodsSelector[
            PaymentMethodsSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.name
        : ""
    );
  }, [id]);
  ...
  const Edit = async () => {
    ...
    await axios
      .put(
        `lab-scope/paymentMethods-update?name=${editedName}&payment_id=${id}`
      )
      .then((response) => {
        GetPaymentMethods();
        setReady(true);
                ...
                <Dialog.Panel className="relative px-10 pb-8 bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-2 sm:max-w-xl sm:w-full ">
                  ...
                  <div className="bg-white  py-3  flex sm:flex-row-reverse justify-between mt-10">
                    <button
                      ...
                      } flex flex-grow font-medium text-sm py-3  justify-center rounded-xl text-white`}
                      onClick={() => Edit()}
                    >
                      {t("Edit Payment Method")}
                    </button>
