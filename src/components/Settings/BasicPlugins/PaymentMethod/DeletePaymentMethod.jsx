...
export default function DeletePaymentMethod({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/paymentMethods-delete?payment_id=${id}`)
      .then((response) => {
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500) setError("this payment method used");
      });
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Payment Method?")} :{" "}
                            <span className="text-black ml-5">
                              {
                                PaymentMethodsSelector[
                                  PaymentMethodsSelector.findIndex(
