...
export default function EditTestMethod({ id, open, setOpen }) {
  ...
  function close() {
    ...
    setEditedName(
      TestMethodSelector
        ? TestMethodSelector[
            TestMethodSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.test_method
        : ""
    );
    setOpen(false);
  ...
  const Edit = async () => {
    ...
    await axios
      .put(
        `lab-scope/update-TestMethod?test_method_id=${id}&test_method=${editedName}`
      )
      .then((response) => {
        GetTestMethods();
        setError("");
