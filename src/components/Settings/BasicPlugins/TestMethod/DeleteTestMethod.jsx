...
export default function DeleteTestMethod({ open, setOpen, id }) {
  ...
  const remove = async () => {
    setReady(false);
    await axios
      .delete(`lab-scope/delete-TestMethod?test_method_id=${id}`)
      .then((response) => {
        GetTestMethods();
        setReady(true);
      ...
      .catch((err) => {
        ...
        if (err.response.status === 500) setError("this Test Method used");
      });
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Test Method?")} :{" "}
