...
export default function DeleteAccountExport({
  UnCheckExport,
  idsExport,
  setIdsExport,
  ...
  const remove = async () => {
    ...
    if (idsExport.length > 0) {
      await axios
        .delete(`lab-scope/accounting-delete?accounting_ids=${idsExport}`)
        .then((response) => {
          setReady(true);
                        ...
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            ...
                            <span className="text-black ml-5">
                              {idsExport.length === 0 && ExportsSelector.data
                                ? ExportsSelector.data[
                                    ExportsSelector.data?.findIndex(
