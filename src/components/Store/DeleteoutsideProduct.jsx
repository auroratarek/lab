...
import { deleteProduct } from "../../api/StoreData";
import swal from "sweetalert";

export default function DeleteoutsideProduct({
  UnCheckOutside,
  idsOutside,
  setIdsOutside,
  ...
  const remove = async () => {
    setReady(false);
    console.log(idsOutside);
    if (idsOutside.length > 0) {
      await axios
        .delete(`lab-scope/outside-delete?out_store_ids=${idsOutside}`)
        .then((response) => {
          // set(response.data);
      ...
      await axios
        .delete(`lab-scope/outside-delete?out_store_ids=${id}`)
        .then((response) => {
          // set(response.data);
          getOutProducts(pageOutside);
          swal("Great!", `Well Done`, "success");
