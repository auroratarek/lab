...
export default function DeleteShowManeReport({ open, setOpen, id }) {
  ...
  const remove = () => {
    let selected =
      ManeReportSelector[
        ManeReportSelector.findIndex((tupeItem) => tupeItem.id === id)
      ];
    dispatch(removeFromEditManeReport(selected));
    setOpen(false);
  };
  return (
    <div>
                      ...
                      <div className={`space-y-5 flex-col `}>
                        <div className="w-full ">
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            {t("Do you want to delete this Range?")} :{" "}
