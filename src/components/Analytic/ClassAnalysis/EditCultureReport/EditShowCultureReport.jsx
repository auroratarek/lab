...
export default function EditShowCultureReport({ open, setOpen, id }) {
  ...
  useEffect(() => {
    setEditedSubject(
      CultureReportSelector
        ? CultureReportSelector[
            CultureReportSelector?.findIndex((unitItem) => unitItem.id === id)
          ]?.text
        : ""
    );
  }, [id]);
  ...
  const Edit = () => {
    let selected =
      CultureReportSelector[
        CultureReportSelector.findIndex((tupeItem) => tupeItem.id === id)
      ];
    let content = CultureReportSelector.map((item) => {
      return item.id == id
        ? { id: id, text: document.getElementById("EditReportName").value }
        : item;
    });
    dispatch(addAllEditCultureReport(content));
    setOpen(false);
  };
