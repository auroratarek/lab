...
export default function DeleteSupplier({
  ...
  const remove = async () => {
    ...
    if (ids.length > 0) {
      await axios
        .delete(`lab-scope/supplier-delete?supplier_ids=${ids}`)
        .then((response) => {
          setReady(true);
      ...
      await axios
        .delete(`lab-scope/supplier-delete?supplier_ids=${id}`)
        .then((response) => {
          setReady(true);
          swal("Great!", `Well Done`, "success");
                    ...
                    <div className="flex flex-col space-y-5 w-full m-auto mt-10">
                      <div className={`space-y-5 flex-col `}>
                          ...
                          <p className="w-fit  font-Poppins-Medium text-sm ">
                            ...
                            <span className="text-black ml-5">
                              {ids.length === 0 &&
                                ...
                                SupplierSelector.data[
                                  SupplierSelector.data.findIndex(
                                    (Item) => Item.id === id
                                  )
                        ...
                        <p className="text-sm text-yellow-500">
                          Warning : if u delete this supplier all the bills and
                          pays that attach to it will be removed
                        </p>
                      </div>
                    </div>
                  </div>
